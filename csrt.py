...
class Dogfight:
    ...
    def capture_frames(self):
        while cap.isOpened():
            ...
            time.sleep(0.02)  # Mimic cv2.waitKey(30)
    ...
    def yolo(self):
        ...
        while True:
            if self.frame is not None:
                if self.tracking_active and self.csrt_tracker is not None:
                    self.csrt()
                else:
                    if self.yolo_active:
                        self.run_yolo()
                ...
                cv2.imshow('YOLOv5 with CSRT', self.frame)
    ...
    def kareciz(self):
        ...
        cv2.rectangle(self.frame, (self.top_left_x, self.top_left_y), (self.bottom_right_x, self.bottom_right_y),
                      (255, 0, 0), 2)
    ...
    def cizgi_cikar(self):
        ...
        for det in self.results.xyxy[0]:
            ...
            cv2.line(self.frame, (int(object_center_x), 0), (int(object_center_x), height), (0, 255, 0), 2)

    def csrt(self):
        if self.csrt_tracker is not None:
            success, bbox = self.csrt_tracker.update(self.frame)
            if success:
                ...
                color = (0, 255, 0)
                thickness = 3
                cv2.rectangle(self.frame, (x, y), (x + w, y + h), color, thickness)

                # Display the coordinates of the bounding box in the top-right corner
                cv2.putText(self.frame, f'X: {x}, Y: {y}', (self.frame.shape[1] - 250, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                cv2.putText(self.frame, f'W: {w}, H: {h}', (self.frame.shape[1] - 250, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

                # Add a label with confidence score
                cv2.putText(self.frame, f'Conf: {self.results.xyxy[0][0][4]:.2f}', (self.frame.shape[1] - 250, 110),
    ...
    def run_yolo(self):
        ...
        for det in detections:
            ...
            # Initialize and start CSRT tracking
            self.csrt_tracker = cv2.TrackerCSRT_create()
            self.csrt_tracker.init(self.frame, bbox)
            self.tracking_active = True
            self.yolo_active = False

            # Display the coordinates of the bounding box
            cv2.putText(self.frame, f'X1: {int(x1)}, Y1: {int(y1)}', (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX,
                        0.7, (0, 255, 0), 2)
            cv2.putText(self.frame, f'X2: {int(x2)}, Y2: {int(y2)}', (int(x2), int(y2) - 10), cv2.FONT_HERSHEY_SIMPLEX,
                        0.7, (0, 255, 0), 2)
    ...
    def fps(self):
        ...
        cv2.putText(self.frame, f'FPS: {int(self.fps_display)}', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0),
                    2)
